...
import {
	type NavItem,
	SectionNav,
} from "@/components/booking/_shared/section-nav";
...
/**
 * <SummaryRail /> — sticky right rail for operations-cluster sub-pages.
 *
 * Shows live event summary + pricing breakdown + optional section
 * progress nav + save/action footer. Desktop only (≥lg); mobile/tablet
 * consumers should keep their existing sticky bottom bar.
 *
 * `width` prop maps to two operations-cluster widths:
 *   - "sm" → 280px (booking form — less content on the rail)
 *   - "md" → 320px (richer summaries — payments, rekap)
 *
 * Renamed from the original <SummaryPanel> at
 * src/components/booking/_shared/summary-panel.tsx — that file now
 * re-exports from here for back-compat until consumers migrate.
 */
...
export interface SummaryRailProps {
	...
	/** Outer width preset — "sm" (280px) default for booking, "md" (320px) for richer pages. */
	width?: "sm" | "md";
}

const WIDTH_CLASS: Record<NonNullable<SummaryRailProps["width"]>, string> = {
	sm: "w-[280px]",
	md: "w-[320px]",
};
		...
		>
			...
			<div className="flex flex-col gap-2 border-t border-border-default pt-4">
				<button
					...
					form="booking-form"
					disabled={pending}
					className="press-down inline-flex h-10 items-center justify-center rounded-md bg-emerald-600 dark:bg-emerald-500 px-4 text-[13px] font-medium text-white transition-colors hover:bg-emerald-700 dark:hover:bg-emerald-600 disabled:opacity-60"
				>
