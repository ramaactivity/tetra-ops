...
export const piutang: AiTool = {
	...
	description:
		...
		"pengingat pelunasan: jatuh_tempo = tenggat pelunasan event (H-1/H-3/H-7 sesuai kesepakatan).",
	scope: "finance",
	parameters: { type: "OBJECT", properties: {} },
	async run(_args, ctx) {
		...
		const { data, error } = await ctx.supabase
			.from("events")
			.select(
				"id, project_id, client_name, client_wa, event_date, due_date, total_paid, remaining_balance, grand_total",
			)
		...
		// Tenggat pelunasan = events.due_date (satu sumber, disinkron dengan
		// invoice; kosong → standar H-1). Invoice dibaca hanya untuk nomornya.
		const { data: docs } = await ctx.supabase
			.from("documents")
			.select("event_id, doc_number")
			.eq("doc_type", "invoice")
			.neq("status", "void")
			.in(
				"event_id",
		...
		const mapped = rows.map((r) => {
			...
			const jatuhTempo = r.due_date ?? addDaysISO(r.event_date, -1);
			return {
