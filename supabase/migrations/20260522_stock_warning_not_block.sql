-- 20260522_stock_warning_not_block.sql
-- ============================================================================
-- Change stock insufficient behavior: WARNING ONLY, no block.
-- ============================================================================
--
-- Rationale (user feedback 2026-05-19):
--   "Stok tidak cukup tidak perlu ada blokir, gunakan mekanisme minus saja.
--    Cukup berikan warning. Nanti kalau ada restock lagi tinggal dikurangi
--    dari minus tersebut."
--
-- Translates to: stock balance allowed to go NEGATIVE. Owner aware via warning,
-- but settlement proceeds. Restock later brings balance back to positive
-- (negatif - delta = balanced).
--
-- Implementation:
--   1. Remove RAISE EXCEPTION on stock shortage di settle_event.
--   2. Log shortage details to audit_log dengan action='stock_warning' supaya
--      tetap traceable kalau ada finding di reconciliation.
--
-- _validate_recap_stock_sufficient function unchanged — masih return JSONB
-- dengan shortages list. Caller (UI) can still display warning.
...
BEGIN
  ...
  -- ------------------------------------------------------------------
  -- Per user direction 2026-05-19: stock allowed to go negative. Just log
  -- warning to audit_log so reconciliation can pick it up. Settlement proceeds.
  ...
  IF NOT (v_stock_check->>'sufficient')::BOOLEAN THEN
    ...
    VALUES (
      'event_settlement',
      NULL,  -- settlement_id not created yet
      'stock_warning',
      jsonb_build_object(
        'event_id', p_event_id,
        'recap_id', v_recap.id,
    ...
    -- Continue to settlement; do not raise.
  END IF;
  ...
  -- 11. Stock deduction (allowed to go negative)
...
COMMENT ON FUNCTION settle_event IS
  'Atomic settle event v2. Auto-derives HPP/OpEx, validates rekap, deducts stock (allowed negative), creates settlement+stock_movements+sinking+owner_earnings+journal_entries. Locks event+recap. Returns JSONB with all generated IDs + stock_shortages array (warning only).';
