-- 20260615_settle_event_count_commissions.sql
-- FIX (HIGH): komisi_vendor & komisi_relasi sebelumnya di-hardcode 0 di
-- settle_event, padahal booking menyimpan events.vendor_commission_amount &
-- events.referrer_commission → komisi tidak pernah jadi OpEx → laba/margin/
-- sinking/owner-pool TER-LEBIH-SAJI untuk event ber-komisi.
--
-- Perubahan: v_opex sekarang ambil komisi dari kolom event (saat tidak di-
-- override). Sisanya 100% identik dgn 20260609. Jurnal + COA sudah siap
-- (5-300 Beban Komisi Vendor, 5-301 Beban Komisi Sales/Relasi; _create_
-- settlement_journal sudah men-debit & v_opex_total sudah memasukkan komisi),
-- jadi tidak ada perubahan lain — jurnal tetap balance.
--
-- Event yg sudah settled tidak berubah (immutable). Berlaku utk settle baru.
