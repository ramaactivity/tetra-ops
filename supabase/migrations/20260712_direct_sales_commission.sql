-- ============================================================================
-- 20260712_direct_sales_commission.sql
--
-- Komisi sales Tetra untuk event channel=direct. Klien bayar penuh; sales
-- (user Tetra yang closing) dapat komisi Rp50-100rb, jadi uang keluar terpisah
-- (beban) seperti komisi relasi.
--
--   1. events.sales_user_id  → user Tetra penerima komisi (dipilih saat booking).
--      (events.direct_sales_commission BIGINT sudah ada sejak awal, default 0.)
--   2. commission_payouts.kind boleh 'sales' (selain vendor/relasi).
--   3. Akun utang komisi 2-102 dipakai bareng relasi & sales → rename jadi
--      'Hutang Komisi Relasi/Sales' (beban tetap 5-301 'Komisi Sales/Relasi').
--   4. settle_event: bucket komisi_sales_direct kini baca
--      events.direct_sales_commission (dulu hardcoded 0 → tak pernah jadi
--      beban/utang). _create_settlement_journal sudah menggabung
--      komisi_sales_direct ke kredit 2-102 (lihat 20260712_commission_payouts).
--
-- Aman: hanya event direct dgn direct_sales_commission>0 yang terpengaruh;
-- event lama semua 0 → P&L & saldo tak berubah.
-- ============================================================================

ALTER TABLE events ADD COLUMN IF NOT EXISTS sales_user_id UUID REFERENCES users(id);
CREATE INDEX IF NOT EXISTS ix_events_sales_user ON events(sales_user_id) WHERE sales_user_id IS NOT NULL;

ALTER TABLE commission_payouts DROP CONSTRAINT IF EXISTS commission_payouts_kind_check;
ALTER TABLE commission_payouts ADD CONSTRAINT commission_payouts_kind_check
  CHECK (kind IN ('vendor', 'relasi', 'sales'));

UPDATE chart_of_accounts
SET name='Hutang Komisi Relasi/Sales',
    description='Utang komisi ke perelasi/sales Tetra, dibayar setelah event'
WHERE code='2-102';

-- settle_event: baca direct_sales_commission ke bucket komisi_sales_direct.

CREATE OR REPLACE FUNCTION public.settle_event(p_event_id uuid, p_owner_user_id uuid, p_overrides jsonb DEFAULT NULL::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
